...
# Set proxy for Hugging Face downloads
    ...
    **kwargs,
    ...
    if tokenizer_mode == "mistral":
        ...
        try:
            ...
            # Special handling for TokenizersBackend - try without tokenizer_class
            if "TokenizersBackend" in str(e) and "does not exist or is not currently imported" in str(e):
                logger.warning(
                    f"TokenizersBackend not found, retrying without "
                    f"tokenizer_class specification")
                # Remove tokenizer_class from kwargs if present
                kwargs_copy = kwargs.copy()
                kwargs_copy.pop('tokenizer_class', None)
                try:
                    tokenizer = AutoTokenizer.from_pretrained(
                        tokenizer_name,
                        *args,
                    ...
                    logger.error(f"Failed to load tokenizer even after removing tokenizer_class: {e2}")
                    raise e2
